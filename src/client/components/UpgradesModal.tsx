...
export default function UpgradesModal({ state, buyUpgrade, buyPassive, buyInfinite, onClose }: Props) {
  return (
        ...
        <section className="upgrade-section">
          ...
          <div className="upgrade-grid">
            {UPGRADES.map(upgrade => {
              ...
              return (
                <div key={upgrade.key} className={`upgrade-card ${canAfford && !maxed ? 'affordable' : ''}`}>
                  <div className="upgrade-header">
                    <h4>{upgrade.label}</h4>
                    <span className="level">Lvl {level}{upgrade.maxLevel ? `/${upgrade.maxLevel}` : ''}</span>
                  </div>
